const {
  execute,
  TacoOperatorsDocument,
  TacoAuthHistoryDocument,
} = require("../../.graphclient");
...
//
// Query the TACo staking subgraph to return the TACo operators that have been
// confirmed before a specific date (i.e. `confirmOperatorAddress` was called
// on TACoApplication contract)
// The graphqlClient passed must be for Polygon staking subgraph
//
async function getOpsConfirmedUntil(endTimestamp) {
  const tacoOperatorsQueryResult = await execute(TacoOperatorsDocument, {
    endTimestamp: endTimestamp,
    first: 1000,
    skip: 0,
  });

  const tacoOperators = tacoOperatorsQueryResult.data.tacoOperators;
...
//
// Query the TACo staking subgraph and return the history of TACo authorization
// changes until a given timestamp.
...
async function getTACoAuthHistoryUntil(endTimestamp) {
  const tacoAuthHistoryQueryResult = await execute(TacoAuthHistoryDocument, {
    endTimestamp: endTimestamp,
    first: 3000,
    skip: 0,
  });

  const appAuthHistories = tacoAuthHistoryQueryResult.data.appAuthHistories;
...
//
// Return the TACo rewards potentially earned by stakes for a period of time.
// It does not apply any penalization, just calculates the rewards based on
// the stake amount and the duration of the stake
//
async function getPotentialRewards(startPeriodTimestamp, endPeriodTimestamp) {
...
//
// Calculate the penalties for the TACo rewards based on the failed heartbeats
...
function calculatePenalties(potentialRewards, failedHeartbeats) {
  ...
  Object.keys(failedHeartbeats).forEach((stProv) => {
    if (!potentialRewards[stProv]) {
      return;
    }

    // If the node failed 2 heartbeats, penalize 1/3 of the reward
    if (failedHeartbeats[stProv].length === 2) {
      earnedTACoRewards[stProv].amount = BigNumber(
        potentialRewards[stProv].amount
      )
        .times(2)
        .div(3)
        .toFixed(0);
