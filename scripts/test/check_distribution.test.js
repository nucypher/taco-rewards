...
describe("Checking last TACo rewards distribution", () => {
  const distFiles = fs.readdirSync(`./distributions`);
  const latestDist = JSON.parse(fs.readFileSync(`./distributions/latest.json`));
  const lastDist = JSON.parse(
    fs.readFileSync(`./distributions/${distFiles[distFiles.length - 2]}`)
  );
  const secondToLastDist = JSON.parse(
    fs.readFileSync(`./distributions/${distFiles[distFiles.length - 3]}`)
  );
  const lastDistDate = new Date(distFiles[distFiles.length - 2].slice(0, -5));
  const secondToLastDistDate = new Date(
    distFiles[distFiles.length - 3].slice(0, -5)
  );

  describe("Checking calculations of the last distribution", () => {
    it("latest and the last distribution files should be identical", () => {
      expect(JSON.stringify(latestDist)).to.be.equal(JSON.stringify(lastDist));
    });

    it("should total distribution amounts be correctly calculated", () => {
      const thisDistributionAmount = Object.keys(latestDist.claims).reduce(
        (acc, key) =>
          acc.plus(BigNumber(latestDist.claims[key].earnedThisDistribution)),
        BigNumber(0)
      );
      const accumulatedAmount = BigNumber(
        secondToLastDist.accumulatedAmount
      ).plus(BigNumber(latestDist.thisDistributionAmount));

      expect(thisDistributionAmount.toFixed(0)).to.be.equal(
        latestDist.thisDistributionAmount
      );
      expect(thisDistributionAmount.eq(latestDist.thisDistributionAmount));
      expect(accumulatedAmount.toFixed(0)).to.be.equal(
        latestDist.accumulatedAmount
      );
    });

    it("should number of claims be equal or greater than in the previous distribution", () => {
      expect(Object.keys(latestDist.claims).length).to.be.gte(
        Object.keys(secondToLastDist.claims).length
      );
    });

    it("should all previous stakes be present in the latest distribution", () => {
      Object.keys(secondToLastDist.claims).forEach((stProv) => {
        expect(
          latestDist.claims[stProv],
          `Stake ${stProv} missing in the latest distribution`
        ).to.not.be.undefined;
      });
    });

    it("should accumulated amount of each stake be correctly calculated", () => {
      Object.keys(latestDist.claims).forEach((stProv) => {
        const previousAccumulatedAmount = secondToLastDist.claims[stProv]
          ? BigNumber(secondToLastDist.claims[stProv].accumulatedAmount)
          : BigNumber(0);
        const earnedThisDistribution = BigNumber(
          latestDist.claims[stProv].earnedThisDistribution
        );
        const expectedAccumulatedAmount = previousAccumulatedAmount
          .plus(earnedThisDistribution)
          .toFixed(0);

        expect(
          latestDist.claims[stProv].accumulatedAmount,
          `Error in accumulated amount for stake ${stProv}`
        ).to.equal(expectedAccumulatedAmount);
      });
    });
    ...
    it("should infractors be penalized", () => {
      Object.keys(latestDist.claims).map((stProv) => {
        ...
        const earnedThisDistribution =
          latestDist.claims[stProv].earnedThisDistribution;
        const penaltyThisDistribution =
          latestDist.claims[stProv].penaltyThisDistribution;
        if (earnedThisDistribution !== "0" || penaltyThisDistribution !== "0") {
          if (failures >= 4) {
            expect(earnedThisDistribution).to.equal(
              "0",
              `Error calculating ${stProv} earned amount`
            );
            expect(
              BigNumber(penaltyThisDistribution).gt(0),
              `Error calculating ${stProv} penalty amount`
            ).to.be.true;
          } else if (failures === 3 || failures === 2) {
            expect(
              BigNumber(earnedThisDistribution).gt(0),
              `Error calculating ${stProv} earned amount`
            ).to.be.true;
            expect(
              BigNumber(penaltyThisDistribution).gt(0),
            ...
            expect(penaltyThisDistribution).to.equal(
              "0",
              `Error calculating ${stProv} penalty amount`
            );
          }
        ...
        it("should rewards for any stake not exceed the corresponding to 15M", () => {
          ...
          Object.keys(latestDist.claims).map((stProv) => {
            const potentialEarning = BigNumber(
              ...
              BigNumber(latestDist.claims[stProv].penaltyThisDistribution)
            );
            expect(
              maxAmount.gte(potentialEarning),
